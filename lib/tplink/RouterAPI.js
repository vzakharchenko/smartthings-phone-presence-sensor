...
const statusUrl = token => `${config
  .router.httpOrhttps}://${config.router
  .routerIp}:${config.router
  .routerPort}/cgi-bin/luci/;stok=${token}/admin/status?form=all`;

const getBlackListUrl = token => `${config
  .router.httpOrhttps}://${config.router
  .routerIp}:${config.router
  .routerPort}/cgi-bin/luci/;stok=${token}/admin/access_control?form=black_list`;

const accessControlUrl = token => `${config
  .router.httpOrhttps}://${config.router
  .routerIp}:${config.router
...
function getRouterInfo(token) {
  return new Promise((resolve, reject) => {
    const url = statusUrl(token.stok);
    sendData(
      `${url}`,
      'POST',
      'operation=read',
...
function getBlackList(token) {
  return new Promise((resolve, reject) => {
    const url = getBlackListUrl(token.stok);
    sendData(
      `${url}`,
      'POST',
      'operation=load',
...
function accessControl(token) {
  return new Promise((resolve, reject) => {
    const url = accessControlUrl(token.stok);
    sendData(
      `${url}`,
      'POST',
      'operation=read',
      ...
      getBlackList(token).then((macs) => {
        ...
        if (findMacs && findMacs.length > 0) {
          ...
          sendData(
            `${url}`,
            'POST',
            `operation=insert&key=add&index=0
          &old=add&new=%7B%22name%22
          %3A%22${`smartThing block${macs.length}`}%22%2C%22mac%22%3A%22${mac}%22%7D`,
            { 'Content-Type': 'application/x-www-form-urlencoded', cookie: token.header },
          )
            .then((res) => {
              const json = JSON.parse(res);
      ...
      getBlackList(token).then((macs) => {
        ...
        if (!findMacs || findMacs.length === 0) {
          ...
          sendData(
            `${url}`,
            'POST',
            `operation=remove&key=key-${pos + 2}&index=${pos}`,
            { 'Content-Type': 'application/x-www-form-urlencoded', cookie: token.header },
          )
            .then((res) => {
              const json = JSON.parse(res);
