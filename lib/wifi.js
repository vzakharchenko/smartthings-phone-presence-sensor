...
function createGuestNetwork(wifiPassword, wifiName, enable) {
  return new Promise((resolve, reject) => {
    ...
    if (wifiPrefix) {
      ...
      guestNetwork[`${wifiPrefix}_wpa_psk`] = guestNetworkClone.psk;
      ...
      .then((asusToken) => {
        sendData(
          ...
          sendData(
            ...
            const responseObject = { availableMacs: availableMacInfo, maclist: availableMacs };
...
function restartQos(asusToken) {
  return new Promise((resolve, reject) => {
    const restartJson = { action_mode: 'apply', rc_service: 'restart_firewall;restart_qos' };
    sendData(
      applyAppGetUrl(), 'POST',
      encodeURIComponent(restartJson),
      {
        cookie: `asus_token=${asusToken.asus_token}`,
        'Content-Type': 'application/x-www-form-urlencoded',
        'User-Agent': 'asusrouter-Android-DUTUtil-1.0.0.155',
