...
function getAsusToken() {
  let basicToken = config.router.encryptedPassword;
  if (!basicToken || config.router.password) {
    config.router.encryptedPassword = null;
    const { userName, password } = config.router;
    ...
    if (!checkToken()) {
      sendData(
        ...
        if (!config.router.encryptedPassword) {
          config.router.encryptedPassword = basicToken;
