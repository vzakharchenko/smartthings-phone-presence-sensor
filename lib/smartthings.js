...
function sendPromise(body, appId, secret) {
  return new Promise((resolve, reject) => {
    const url = `${apiGateWay()}${appId}${apiGateWayStatus()}${secret}`;
    sendData(
      url,
      'POST',
      JSON.stringify(body),
  ...
  return new Promise((resolve, reject) => {
    ...
    Object.keys(smartthings).forEach((name) => {
      if (name) {
        ...
        promises.push(sendPromise(body, user.appId, user.secret));
      }
    });
...
function apiGateWayInit(body, userName, appId, secret) {
  return new Promise((resolve, reject) => {
    sendDataWithResponse(
      `${apiGateWay()}${appId}${apiGateWayInitUrl()}${secret}`,
      'POST',
      JSON.stringify(body),
      {
        'Content-Type': 'application/json',
